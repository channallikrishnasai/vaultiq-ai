...
import {
  PiggyBank, Plus, Trash2, PieChart, Info, RefreshCw, AlertTriangle
} from "lucide-react";
      ...
      <div className="flex items-center justify-between mb-4 border-b border-zinc-800/40 pb-4 shrink-0">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-0.5">
            Configure monthly category limits, setup warnings, and view spending thresholds
          </p>
        </div>
        <button
          onClick={fetchData}
        ...
        <div className="col-span-5 space-y-4">
          ...
          <div className="p-4 rounded-xl border border-zinc-800/60 bg-zinc-900/30 backdrop-blur-md">
            ...
            <p className="text-[10px] text-zinc-500 leading-relaxed">
              VaultIQ AI automatically monitors envelope budgets and alerts you if expenses exceed 85% of set limits.
            </p>
          </div>
        </div>

        {/* Right Column: Envelope Status List (7 Cols) */}
        <div className="col-span-7 p-4 rounded-xl border border-zinc-800/60 bg-zinc-900/30 backdrop-blur-md flex flex-col overflow-hidden">
          ...
          {budgets.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center py-16 border border-dashed border-zinc-800 rounded-lg">
              ...
              <p className="text-xs text-zinc-500">No active budgets set for this period.</p>
            </div>
          ) : (
            <div className="flex-1 overflow-y-auto scrollbar-thin space-y-3 pr-1">
              {budgets.map((b) => {
                ...
                const isWarn = spent > b.limit * 0.85;

                return (
                  <div key={b.id} className="p-3 rounded-lg border border-zinc-800/40 bg-zinc-900/20 space-y-2">
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="text-xs font-bold text-white block">{b.category}</span>
                        <span className="text-[9px] text-zinc-500">Month: {b.month}/{b.year}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="text-right">
                          ...
                          <span className={`text-[9px] font-bold ${isOver ? "text-rose-400" : isWarn ? "text-amber-400" : "text-emerald-400"}`}>
