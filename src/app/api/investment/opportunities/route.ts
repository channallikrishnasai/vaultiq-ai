...
export async function GET(request: NextRequest) {
  try {
    ...
    const rateLimitResult = checkRateLimit(`investment-opportunities:${session.user.id}`, RATE_LIMITS.analysis);
    ...
    const opportunities = await investmentAdvisorService.getOpportunities(
      session.user.id,
      portfolioId
    );
