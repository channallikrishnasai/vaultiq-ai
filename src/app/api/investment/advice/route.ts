...
export async function GET(request: NextRequest) {
  try {
    ...
    const rateLimitResult = checkRateLimit(`investment-advice:${session.user.id}`, RATE_LIMITS.analysis);
    ...
    const advice = await investmentAdvisorService.getInvestmentAdvice({
